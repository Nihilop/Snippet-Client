      ...
      <n-card :border="false">
        <div class="save_card">
          ...
          <n-space class="btn_wrapper">
            ...
            <n-button type="success" @click="createPoste">{{$t('utils.save')}}</n-button>
          </n-space>
        </div>
      </n-card>
    </div>
...
import { NForm, NFormItem, NSpace, NThing, NButton, NH1, NInput, NSelect, NAvatar, NCard, NSwitch } from 'naive-ui'
export default {
  components: {
    NForm,
    NFormItem,
  ...
  setup (props, { emit }) {
    ...
    onMounted(() => {
      user.value.author = store.state.user.currentUser.name
      user.value.avatar = store.state.user.currentUser.avatar
      user.value.email = store.state.user.currentUser.email
    })
    ...
    function createPoste (e) {
      e.preventDefault()
      formRef.value.validate((errors) => {
        if (!errors) {
          console.log(modelRef.value)
          store.dispatch('blog/CREATE', modelRef.value)
